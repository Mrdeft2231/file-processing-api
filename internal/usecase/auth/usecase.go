...
// Register - регистрация нового пользователя
func (uc *auth) Register(username string, password string) (string, error) { // Проверка сложности и длины пароля
	if len(password) < 6 || len(password) > 128 {
		return "", ErrMinLengthPswd
	}

	// Проверяем, что пользователя с таким username не существует
	existingUser, err := uc.userRepo.GetUserByUsername(context.Background(), username)
